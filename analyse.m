...
realNews = readtable('True.xlsx');
fakeNews = readtable('Fake.xlsx');

sizeFakeNews=int16(size(fakeNews, 1));
sizeRealNews=int16(size(realNews, 1));

realNews = [realNews, array2table(ones(sizeRealNews))];
...
fakeNewsTrain = fakeNews(1:sizeFakeNews*fracTrain,:);
fakeNewsVerif = fakeNews(sizeFakeNews*(1-fracTrain):sizeFakeNews,:);
realNewsTrain = realNews(1:sizeRealNews*fracTrain,:);
realNewsVerif = realNews(sizeRealNews*(1-fracTrain):sizeRealNews,:);
...
% 
% %On cree le classificateur k plus provhes voisin
...
% 
% %On cree et on train le reseau de neurone
